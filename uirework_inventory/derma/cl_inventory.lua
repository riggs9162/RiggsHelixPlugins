...
hook.Add("CreateMenuButtons", "ixInventory", function(tabs)
	...
	tabs["inv"] = {
		...
		Create = function(info, container)
			local model = container:Add("ixModelPanel")
			model:Dock(RIGHT)
			model:SetWide(ScrW() * 0.25)
			model:SetModel(LocalPlayer():GetModel(), LocalPlayer():GetSkin())
			model:SetFOV(11)
			model:SetCamPos(Vector(-90, 180, 90))
			model:SetLookAt(Vector(0, 0, 35))

			model.PaintOver = function(this, width, height)
				if not ( IsValid(ix.gui.menu) ) then
					return
				end

				if not ( ix.gui.menu.bClosing ) then
					return
				end

				local alpha = ix.gui.menu.currentAlpha
				this:SetAlpha(alpha)
			end
